...
cat("\n=== EPIC Immune Infiltration Analysis ===\n")
...
cache_file <- file.path(cache_dir, "epic.qs2")
...
if (file.exists(cache_file) && !opt$force) {
  ...
  formatted_time()
  epic_res <- qs_read(cache_file)
  cat("Loaded cached EPIC result\n")
} else {
  cat("Running EPIC...\n")
  formatted_time()
  epic_res <- deconvo_tme(eset = dat, method = "epic", arrays = opt$arrays)
  
  cat("Saving to cache...\n")
  qs_save(epic_res, cache_file)
  cat(sprintf("Saved to: %s\n", cache_file))
  formatted_time()
}

names(epic_res) <- gsub("_EPIC$", "", names(epic_res))
fwrite(epic_res, file.path(temp_outdir, "01.epic_res.csv"))
cat(sprintf("Saved EPIC results: %s\n", file.path(temp_outdir, "01.epic_res.csv")))

cell_cols <- setdiff(colnames(epic_res), c("ID", "otherCells"))
if (length(cell_cols) > 0) {
  mypalette <- colorRampPalette(brewer.pal(min(length(cell_cols), 9), "Set1"))

  p <- epic_res %>%
    gather(cell_type, fraction, -ID, -otherCells) %>%
  ...
  save_plot('01.epic.stacked_bar', p, outdir = temp_outdir, width = 10, height = 6, both = TRUE)
  cat("Saved stacked bar plot\n")
}

res_epic2 <- epic_res %>%
  dplyr::select(-c("otherCells")) %>%
  column_to_rownames(var = 'ID') %>%
  t() %>%
  as.data.frame()
res_epic2 <- res_epic2[rowSums(res_epic2) > 0, ]

dat_epic <- res_epic2 %>%
  t %>%
  as.data.frame() %>%
  tibble::rownames_to_column(var = "ID")
dat_epic <- merge(df.group, dat_epic, by = "ID")
dat_epic2 <- tidyr::gather(dat_epic, ImmuneCell, Score, -c("ID", "group"))

# Filter out NA values and cell types with no variance
dat_epic2 <- dat_epic2 %>% filter(!is.na(Score) & !is.nan(Score) & Score != Inf & Score != -Inf)

# Filter to only include cell types that have data in both groups
cell_types_valid <- dat_epic2 %>%
  group_by(ImmuneCell) %>%
  summarise(n_groups = n_distinct(group), .groups = 'drop') %>%
  filter(n_groups >= 2) %>%
  pull(ImmuneCell)

dat_epic2 <- dat_epic2 %>% filter(ImmuneCell %in% cell_types_valid)

if (nrow(dat_epic2) > 0 && length(unique(dat_epic2$group)) >= 2) {
  stat_epic <- dat_epic2 %>%
    group_by(ImmuneCell) %>%
  ...
  stat_epic <- data.frame()
}

fwrite(stat_epic, file.path(temp_outdir, '02.stat.epic.csv'))
cat("Saved statistical analysis\n")

DE_epic <- stat_epic

violin_epic <- dat_epic2

if (nrow(violin_epic) > 0 && nrow(DE_epic) > 0) {
  p1 <- ggplot(violin_epic, aes(x = ImmuneCell, y = Score, fill = group)) +
    geom_boxplot(width = 0.5,
                 alpha = 0.8,
                 position = position_dodge(0.9),
                 outlier.shape = NA) +
    scale_fill_manual(values = group_color[1:2]) +
    annotate(geom = "text", x = DE_epic$ImmuneCell,
             y = max(violin_epic$Score), size = 3,
             label = as.character(DE_epic$p.signif)) +
    theme_classic() +
  ...
  save_plot('03.DE.epic.boxplot', p1, outdir = temp_outdir, width = 12, height = 7, both = TRUE)
  cat("Saved boxplot\n")
} else {
...
if (!is.null(opt$genes)) {
  ...
  DE_epic <- stat_epic[which(stat_epic$p < 0.05), ]
  
  if (nrow(DE_epic) > 0) {
    cortiic <- cortiic[DE_epic$ImmuneCell, ]
    if (ncol(cortiic) > 0) {
