...
cat("\n=== xCell Immune Infiltration Analysis ===\n")
...
cache_file <- file.path(cache_dir, "xcell.qs2")
...
if (file.exists(cache_file) && !opt$force) {
  ...
  formatted_time()
  xcell_res <- qs_read(cache_file)
  cat("Loaded cached xCell result\n")
} else {
  cat("Running xCell...\n")
  formatted_time()
  xcell_res <- deconvo_tme(eset = dat, method = "xcell", arrays = opt$arrays)
  
  cat("Saving to cache...\n")
  qs_save(xcell_res, cache_file)
  cat(sprintf("Saved to: %s\n", cache_file))
  formatted_time()
}

names(xcell_res) <- gsub("_xCell$", "", names(xcell_res))
fwrite(xcell_res, file.path(temp_outdir, "01.xcell_res.csv"))
cat(sprintf("Saved xCell results: %s\n", file.path(temp_outdir, "01.xcell_res.csv")))

cell_cols <- setdiff(colnames(xcell_res), "ID")
if (length(cell_cols) > 0) {
  ...
  p <- xcell_res %>%
    ...
    theme_bw() +
    labs(x = '', y = 'Enrichment Score', fill = '') +
  ...
  save_plot('01.xcell.stacked_bar', p, outdir = temp_outdir, width = 10, height = 6, both = TRUE)
  cat("Saved stacked bar plot\n")
}

res_xcell2 <- xcell_res %>%
  column_to_rownames(var = 'ID') %>%
  t() %>%
  as.data.frame()
res_xcell2 <- res_xcell2[rowSums(res_xcell2) > 0, ]

dat_xcell <- res_xcell2 %>%
  t %>%
  as.data.frame() %>%
  tibble::rownames_to_column(var = "ID")
dat_xcell <- merge(df.group, dat_xcell, by = "ID")
dat_xcell2 <- tidyr::gather(dat_xcell, ImmuneCell, Score, -c("ID", "group"))

stat_xcell <- dat_xcell2 %>%
  group_by(ImmuneCell) %>%
  wilcox_test(Score ~ group) %>%
  adjust_pvalue(method = "BH") %>%
  add_significance("p")

fwrite(stat_xcell, file.path(temp_outdir, '02.stat.xcell.csv'))
cat("Saved statistical analysis\n")

DE_xcell <- stat_xcell

violin_xcell <- dat_xcell2

p1 <- ggplot(violin_xcell, aes(x = ImmuneCell, y = Score, fill = group)) +
  geom_boxplot(width = 0.5,
               alpha = 0.8,
               position = position_dodge(0.9),
               outlier.shape = NA) +
  scale_fill_manual(values = group_color[1:2]) +
  annotate(geom = "text", x = DE_xcell$ImmuneCell,
           y = max(violin_xcell$Score), size = 3, 
           label = as.character(DE_xcell$p.signif)) +
  theme_classic() +
...
save_plot('03.DE.xcell.boxplot', p1, outdir = temp_outdir, width = 12, height = 7, both = TRUE)
cat("Saved boxplot\n")
...
if (!is.null(opt$genes)) {
  ...
  DE_xcell <- stat_xcell[which(stat_xcell$p < 0.05), ]
  
  if (nrow(DE_xcell) > 0) {
    cortiic <- cortiic[DE_xcell$ImmuneCell, ]
    if (ncol(cortiic) > 0) {
